...
const { calculateBuyerPayment, calculateSellerPayout } = require('../utils/feeCalculator');
    ...
    try {
        ...
        if (orderData.status === 'inprogress') {
            ...
            if (!recruiterId && orderData.chat_id) {
                ...
                try {
                    ...
                    if (existingChat) {
                        ...
                        recruiterId = existingChat.participants.find(p => p.toString() !== createdBy.toString());
            ...
            try {
                ...
                console.log('Chat created successfully with cardData for order:', tempOrder._id);
...
const updateOrderStatus = async (req, res) => {
    try {
        ...
        if (message != (undefined || null)) {
            updatedOrder = await orderService.updateOrderStatus(orderId, status, message, req.user._id, 'status_changed');
        } else {
            updatedOrder = await orderService.updateOrderStatus(orderId, status, " ", req.user._id, 'status_changed');
        }
...
const acceptOrder = async (req, res) => {
    ...
    const updatedOrder = await orderService.updateOrderStatus(orderId, 'accepted', 'Order accepted', userId, 'accepted');
...
const declineOrder = async (req, res) => {
    ...
    const updatedOrder = await orderService.updateOrderStatus(orderId, 'cancel', reason || 'Order declined', userId, 'declined');
...
const processOrderPayment = async (req, res) => {
    try {
        ...
        const { amount, currency = 'USD', paymentMethod, billingAddress, savePaymentInfo, stripePaymentMethodId } = req.body;
        
        console.log('Processing order payment:', { orderId, userId, amount, paymentMethod, stripePaymentMethodId });
        
        // Validate userId exists
        if (!userId) {
            throw new ApiError(httpStatus.UNAUTHORIZED, 'User ID not found in request');
        }
        ...
        if (parseFloat(amount) !== buyerPayment.totalAmount) {
            throw new ApiError(httpStatus.BAD_REQUEST, `Payment amount does not match calculated total. Expected: $${buyerPayment.totalAmount}, Received: $${amount}`);
        }
        ...
        if (!paymentResult || !paymentResult.success) {
            throw new ApiError(httpStatus.PAYMENT_REQUIRED, 'Payment processing failed');
        }
...
const addReviewAndRating = async (req, res) => {
    try {
        ...
        if (rating && typeof rating !== 'number' || (rating && (rating < 1 || rating > 5))) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'Rating must be a number between 1 and 5');
        }

        // Validate new dual rating system
        if (buyerRating && (typeof buyerRating !== 'number' || buyerRating < 1 || buyerRating > 5)) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'Buyer rating must be a number between 1 and 5');
        }

        if (sellerRating && (typeof sellerRating !== 'number' || sellerRating < 1 || sellerRating > 5)) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'Seller rating must be a number between 1 and 5');
        }

        // Validate review lengths for dual system
        if (buyerReview && (buyerReview.length < 10 || buyerReview.length > 200)) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'Buyer review must be between 10 and 200 characters');
        }

        if (sellerReview && (sellerReview.length < 10 || sellerReview.length > 200)) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'Seller review must be between 10 and 200 characters');
        }

        // Call the service to add review and rating
        const updatedOrder = await orderService.addReviewAndRating(orderId, { 
            rating, review, tip, actorId: req.user._id,
            buyerRating, sellerRating, buyerReview, sellerReview 
        });
...
const sendOrderMessage = async (req, res) => {
    try {
        ...
        if (!message || typeof message !== 'string' || !message.trim()) {
            return res.status(httpStatus.BAD_REQUEST).send({ message: 'Message is required' });
        }
        ...
        const recruiterStr = order.recruiterId ? order.recruiterId.toString() : null;
        const userIdStr = userId.toString();
        if (userIdStr !== createdByStr && userIdStr !== recruiterStr) {
            return res.status(httpStatus.FORBIDDEN).send({ message: 'Not allowed to message on this order' });
        }
        ...
        return res.status(httpStatus.CREATED).send({ success: true, orderId: order._id });
    } catch (error) {
...
const requestExtension = async (req, res) => {
    try {
        ...
        if (!order) return res.status(httpStatus.NOT_FOUND).send({ message: 'Order not found' });
        const uid = userId.toString();
        if (uid !== order.createdBy.toString() && uid !== order.recruiterId.toString()) {
            return res.status(httpStatus.FORBIDDEN).send({ message: 'Not allowed to request extension' });
        }
        const updated = await orderService.requestExtension(orderId, Number(days), reason, userId);
...
const decideExtension = async (req, res) => {
    ...
    const order = await Order.findById(orderId).select('createdBy recruiterId extensions');
        if (!order) return res.status(httpStatus.NOT_FOUND).send({ message: 'Order not found' });
        const uid = userId.toString();
        if (uid !== order.createdBy.toString() && uid !== order.recruiterId.toString()) {
            return res.status(httpStatus.FORBIDDEN).send({ message: 'Not allowed to decide extension' });
        }
    const updated = await orderService.decideLatestPending(orderId, decision, userId);
        return res.status(httpStatus.OK).send(updated);
    } catch (error) {
        return res.status(httpStatus.BAD_REQUEST).send({ message: error.message });
    }
...
const extendDelivery = async (req, res) => {
    ...
    if (!Number.isFinite(reqDays) || reqDays <= 0) {
            return res.status(httpStatus.BAD_REQUEST).send({ message: 'extraDays must be a positive number' });
        }
        ...
        const uid = userId.toString();
        if (uid !== order.createdBy.toString() && uid !== order.recruiterId.toString()) {
            return res.status(httpStatus.FORBIDDEN).send({ message: 'Not allowed to extend this order' });
        }
    // Create a pending extension request instead of immediately extending
    const updated = await orderService.requestExtension(orderId, reqDays, reason, userId);
    return res.status(httpStatus.OK).send(updated);
    } catch (error) {
        return res.status(httpStatus.BAD_REQUEST).send({ message: error.message });
    }
...
const requestCancellation = async (req, res) => {
    try {
        ...
        const uid = userId.toString();
        if (uid !== order.createdBy.toString() && uid !== order.recruiterId.toString()) {
            return res.status(httpStatus.FORBIDDEN).send({ message: 'Not allowed to request cancellation' });
        }
        ...
        if (req.files && req.files.length > 0) {
            for (const file of req.files) {
                try {
                    ...
                    console.error('Error uploading cancellation attachment to S3:', uploadError);
                    return res.status(httpStatus.BAD_REQUEST).send({ 
        ...
        const updated = await orderService.requestCancellation(orderId, reason, userId, cancellationAttachments);
        return res.status(httpStatus.OK).send(updated);
    } catch (error) {
        return res.status(httpStatus.BAD_REQUEST).send({ message: error.message });
    }
...
const decideCancellation = async (req, res) => {
    try {
        ...
        const uid = userId.toString();
        if (uid !== order.createdBy.toString() && uid !== order.recruiterId.toString()) {
            return res.status(httpStatus.FORBIDDEN).send({ message: 'Not allowed to decide cancellation' });
        }
        const updated = await orderService.decideLatestCancel(orderId, decision, userId);
        return res.status(httpStatus.OK).send(updated);
    } catch (error) {
        return res.status(httpStatus.BAD_REQUEST).send({ message: error.message });
    }
...
const directCancelOrder = async (req, res) => {
    try {
        ...
        const updated = await orderService.directCancelOrder(orderId, reason, userId);
        return res.status(httpStatus.OK).send(updated);
    } catch (error) {
...
const submitDelivery = async (req, res) => {
    try {
        ...
        if (uid !== createdByStr && uid !== recruiterStr) {
            return res.status(httpStatus.FORBIDDEN).send({ message: 'Not allowed to submit delivery for this order' });
        }
        ...
        order.activities.push({
            ...
            toStatus: 'delivered',
            meta: {
                filesUploaded: deliveryFiles.map(f => ({ filename: f.filename, url: f.url }))
            }
        });
...
const acceptDelivery = async (req, res) => {
    try {
        ...
        const recruiterStr = order.recruiterId? order.recruiterId.toString() : null;
        if (uid !== createdByStr && uid !== recruiterStr) {
            return res.status(httpStatus.FORBIDDEN).send({ message: 'Not allowed to accept this delivery' });
        }
        ...
        if (seller) {
            ...
            console.log(`✅ DELIVERY ACCEPTED: Added $${sellerPayout.toFixed(2)} to seller ${seller.email} (Order: $${orderAmount}, Platform Fee: $${platformFee.toFixed(2)}, VAT: $${vatAmount.toFixed(2)})`);
        }
        ...
        const updatedOrder = await orderService.updateOrderStatus(orderId, 'complete', note, userId, 'delivery_accepted');
...
const getRejectedCancellationOrders = async (req, res) => {
    try {
        ...
        const data = orders.map(o => {
            const adminReview = (o.cancellations || []).filter(c => c.status === 'admin_review');
            const latest = adminReview.sort((a,b) => new Date(b.decidedAt || b.requestedAt) - new Date(a.decidedAt || a.requestedAt))[0];
            return {
                orderId: o._id,
                ...
                declinedBy: latest && latest.declinedByName ? latest.declinedByName : 'Unknown',
                declinedAt: latest && (latest.decidedAt || latest.requestedAt) ? (latest.decidedAt || latest.requestedAt) : null,
        ...
        return res.status(httpStatus.BAD_REQUEST).send({ success: false, message: error.message });
    }
};
...
const adminAcceptCancellation = async (req, res) => {
    try {
        ...
        if (!adminReviewCancellation) {
            return res.status(httpStatus.BAD_REQUEST).send({ 
                success: false, 
                message: 'No cancellation request under admin review found for this order' 
            });
        }
        ...
        adminReviewCancellation.adminReason = adminReason || 'Admin approved cancellation';
...
const adminRejectCancellation = async (req, res) => {
    try {
        ...
        adminReviewCancellation.decidedAt = new Date();
        adminReviewCancellation.decidedBy = adminId;
        adminReviewCancellation.adminReason = adminReason || 'Admin rejected cancellation';
...
const requestDeliveryRevision = async (req, res) => {
    try {
        ...
        const recruiterStr = order.recruiterId? order.recruiterId.toString() : null;
        if (uid !== createdByStr && uid !== recruiterStr) {
            return res.status(httpStatus.FORBIDDEN).send({ message: 'Not allowed to request revision for this order' });
        }
        ...
        order.activities.push({
            ...
            toStatus: 'revision',
            meta: {
                filesUploaded: uploadedFiles.map(f => ({ filename: f.filename, url: f.url })),
            },
        });
