...
const getOrderById = async (orderId, currentUser) => {
    const order = await Order.findById(orderId)
        .populate({ path: 'createdBy', select: 'name email address phone profilePicture bio role' })
        .populate({ path: 'recruiterId', select: 'name email address phone profilePicture bio role' })
        .populate({ path: 'gig', select: 'title description category packages seller' })
        .populate({ path: 'gigId', select: 'title description category packages seller' });
    ...
    const createdById = order.createdBy && order.createdBy._id ? order.createdBy._id.toString() : null;
    const recruiterId = order.recruiterId && order.recruiterId._id ? order.recruiterId._id.toString() : null;
    const currentUserId = currentUser._id ? currentUser._id.toString() : (currentUser.id ? currentUser.id.toString() : null);
    ...
    let myRole = amCreator ? 'recruiter' : amRecruiter ? 'user' : currentUser.role;
    
    
    // Fetch UserSpace for both sides (if available)
    const createdByIdStr = order.createdBy ? order.createdBy._id.toString() : null;
    const recruiterIdStr = order.recruiterId ? order.recruiterId._id.toString() : null;
    const [createdBySpace, recruiterSpace] = await Promise.all([
        createdByIdStr ? UserSpace.findOne({ createdBy: createdByIdStr }).lean() : null,
        recruiterIdStr ? UserSpace.findOne({ createdBy: recruiterIdStr }).lean() : null,
    ]);
    ...
    if (currentUser.role === 'recruiter') {
        ...
        otherUser = order.createdBy ? {
            ...
            firstName: createdBySpace && createdBySpace.firstName ? createdBySpace.firstName : '',
            lastName: createdBySpace && createdBySpace.lastName ? createdBySpace.lastName : '',
        ...
        otherUser = order.recruiterId ? {
            ...
            firstName: recruiterSpace && recruiterSpace.firstName ? recruiterSpace.firstName : '',
            lastName: recruiterSpace && recruiterSpace.lastName ? recruiterSpace.lastName : '',
    ...
    orderData.activities = await Promise.all(
        (order.activities || []).map(async (activity) => {
            if (activity.by) {
                const userSpace = await UserSpace.findOne({ createdBy: activity.by.toString() }).lean();
                return {
                    action: activity.action,
                    by: activity.by,
                    firstName: userSpace && userSpace.firstName ? userSpace.firstName : '',
...
const updateOrderStatus = async (orderId, status, message, actorId = null, actionType = 'status_changed') => {
    const order = await Order.findById(orderId);
    if (!order) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Order not found');
    }
    ...
    if (status === 'complete' && order.gig) {
        try {
            ...
            if (gig) {
                ...
                gig.totalEarnings = (gig.totalEarnings || 0) + (order.totalAmount || order.price || 0);
                
                await gig.save();
                console.log(`✅ Updated gig stats via updateOrderStatus: totalOrders=${gig.totalOrders}, totalEarnings=${gig.totalEarnings}`);
            }
        } catch (error) {
...
const addReviewAndRating = async (orderId, { 
    rating, review, tip, actorId = null,
    buyerRating, sellerRating, buyerReview, sellerReview 
}) => {
    ...
    if (order.status === 'complete' && order.gig) {
        try {
            ...
            if (gig) {
                ...
                await gig.save();
                console.log(`✅ Updated gig stats: totalOrders=${gig.totalOrders}, totalEarnings=${gig.totalEarnings}`);
            }
        } catch (error) {
...
const getMyOrders = async (user) => {
    ...
    for (const o of orders) {
        const createdById = o.createdBy && o.createdBy._id ? o.createdBy._id.toString() : null;
        const recruiterId = o.recruiterId && o.recruiterId._id ? o.recruiterId._id.toString() : null;
        if (createdById === meId && recruiterId) targetUserIds.add(recruiterId);
        else if (recruiterId === meId && createdById) targetUserIds.add(createdById);
    }
    const spaces = await UserSpace.find({ createdBy: { $in: Array.from(targetUserIds) } }).lean();
    ...
    const formattedOrders = orders.map((order) => {
        ...
        orderData.otherUser = otherUser;
        orderData.otherUserIsCurrentUser = otherUser && otherUser.id && otherUser.id.toString ? otherUser.id.toString() === meId : false; // true only in degenerate cases
        orderData.myRole = myRole;
        return orderData;
    });
...
const getCompletedOrders = async (user) => {
    ...
    const recruiterIds = orders.map(o => o.recruiterId && o.recruiterId._id ? o.recruiterId._id : null).filter(Boolean).map(id => id.toString());
    const spaces = await UserSpace.find({ createdBy: { $in: recruiterIds } }).lean();
    const spaceMap = new Map(spaces.map(s => [s.createdBy.toString(), s]));

    // Map through the orders to include tip and recruiter name
    const formattedOrders = orders.map((order) => {
        const s = order.recruiterId ? spaceMap.get(order.recruiterId._id.toString()) : null;
        const buyerName = s && s.firstName && s.lastName ? `${s.firstName} ${s.lastName}` : (order.recruiterId && order.recruiterId.name ? order.recruiterId.name : 'Unknown');
        return {
            title: order.title,
            price: order.price,
...
module.exports = {
    ...
    async extendDelivery(orderId, extraDays, actorId) {
        if (!Number.isFinite(extraDays) || extraDays <= 0) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'extraDays must be a positive number');
        }
        const order = await Order.findById(orderId);
        if (!order) {
            throw new ApiError(httpStatus.NOT_FOUND, 'Order not found');
        ...
        order.activities.push({
            ...
            meta: { addedDays: extraDays, previous: prev, newTotal: order.delivery_time }
        });
        await order.save();
        return order;
    },
    async requestExtension(orderId, days, reason, actorId) {
        if (!Number.isFinite(days) || days <= 0) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'days must be a positive number');
        }
        const order = await Order.findById(orderId);
        if (!order) throw new ApiError(httpStatus.NOT_FOUND, 'Order not found');
        order.extensions = order.extensions || [];
        const ext = { days, reason, requestedBy: actorId, status: 'pending', requestedAt: new Date() };
        order.extensions.push(ext);
        const extId = order.extensions[order.extensions.length - 1] && order.extensions[order.extensions.length - 1]._id ? order.extensions[order.extensions.length - 1]._id : ext._id; // subdoc _id assigned by mongoose
        order.activities = order.activities || [];
        order.activities.push({
    ...
    async decideExtension(orderId, extensionIndex, decision, deciderId) {
        const order = await Order.findById(orderId);
        if (!order) throw new ApiError(httpStatus.NOT_FOUND, 'Order not found');
        if (!Array.isArray(order.extensions) || extensionIndex < 0 || extensionIndex >= order.extensions.length) {
        ...
        if (decision === 'accepted') {
            ...
            order.activities.push({
                ...
                meta: { addedDays: ext.days, previous: prev, newTotal: order.delivery_time, extensionIndex }
            });
        } else {
            order.activities = order.activities || [];
            order.activities.push({
        ...
        if (extIdStr) {
            ...
            order.activities = order.activities.filter(a => {
                ...
                const aExtId = a.meta && a.meta.extensionId ? String(a.meta.extensionId) : null;
    ...
    async decideExtensionById(orderId, extensionId, decision, deciderId) {
        ...
        const idx = order.extensions.findIndex(e => e._id && e._id.toString() === String(extensionId));
        if (idx === -1) throw new ApiError(httpStatus.BAD_REQUEST, 'Extension not found');
        // Reuse logic
    ...
    async decideLatestPending(orderId, decision, deciderId) {
        ...
        for (let i = order.extensions.length - 1; i >= 0; i--) {
            if (order.extensions[i].status === 'pending') { idx = i; break; }
        }
        if (idx === -1) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'No pending extension to decide');
        }
        return this.decideExtension(orderId, idx, decision, deciderId);
    },
    async requestCancellation(orderId, reason, actorId, attachments = []) {
        ...
        const cancelId = order.cancellations[order.cancellations.length - 1] && order.cancellations[order.cancellations.length - 1]._id ? order.cancellations[order.cancellations.length - 1]._id : req._id;
        order.activities = order.activities || [];
        order.activities.push({
    ...
    async decideLatestCancel(orderId, decision, deciderId) {
        const order = await Order.findById(orderId).populate('createdBy', 'name').populate('recruiterId', 'name');
        if (!order) throw new ApiError(httpStatus.NOT_FOUND, 'Order not found');
        if (!Array.isArray(order.cancellations) || order.cancellations.length === 0) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'No cancellations to decide');
        }
        let idx = -1;
        for (let i = order.cancellations.length - 1; i >= 0; i--) {
            if (order.cancellations[i].status === 'pending') { idx = i; break; }
        }
        if (idx === -1) throw new ApiError(httpStatus.BAD_REQUEST, 'No pending cancellation to decide');
        ...
        const deciderName = deciderId.toString() === order.createdBy._id.toString() 
        ...
        if (cancelIdStr) {
            ...
            order.activities = order.activities.filter(a => {
                ...
                const aId = a.meta && a.meta.cancellationId ? String(a.meta.cancellationId) : null;
    ...
    async canCancelOrder(orderId, userId) {
        ...
        const isCreator = order.createdBy && order.createdBy.toString() === userId.toString();
        const isRecruiter = order.recruiterId && order.recruiterId.toString() === userId.toString();
        ...
        if (order.status === 'delivered' || order.status === 'complete') {
            throw new ApiError(httpStatus.BAD_REQUEST, 'Cannot cancel order after successful delivery. Please contact support for assistance.');
        }

        // Check if there's already a pending cancellation
        const pendingCancellation = order.cancellations && order.cancellations.find(c => c.status === 'pending');
        if (pendingCancellation) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'There is already a pending cancellation request for this order');
        }
        ...
        try {
            ...
            if (order.refundProcessed) {
                ...
                order.activities.push({
                    ...
                    meta: { refundAmount: order.refundAmount, processedAt: order.refundProcessedAt }
                });
                return;
            }
            ...
            if (!order.payment || order.paymentStatus !== 'paid') {
                console.log('⚠️ Order was not paid, but processing refund anyway for testing');
                console.log('📊 Order details:', {
            ...
            const refundAmount = parseFloat(order.payment && order.payment.amount ? order.payment.amount : (order.price || order.totalAmount));
            ...
            console.log(`💰 BUYER BALANCE BEFORE REFUND: $${buyerBefore.balance || 0}`);
            ...
            console.log(`✅ REFUND PROCESSED: Added $${refundAmount.toFixed(2)} to buyer balance for cancelled order ${order._id}`);
            
            // Log refund activity
            order.activities.push({
                action: 'refund_processed',
                by: actorId,
                note: `Refund of $${refundAmount.toFixed(2)} processed to buyer balance`,
                meta: {
    ...
    async directCancelOrder(orderId, reason, actorId) {
        ...
        console.log(`📊 ORDER PAYMENT INFO:`, {
            hasPayment: !!order.payment,
            paymentStatus: order.paymentStatus,
            paymentAmount: order.payment && order.payment.amount ? order.payment.amount : null,
