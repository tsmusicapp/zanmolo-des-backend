...
const orderSchema = mongoose.Schema(
  {
    ...
    status: {
      type: String,
      enum: ['pending_payment', 'active', 'inprogress', 'accepted', 'delivered', 'revision', 'cancel', 'complete'],
      default: "active",
      required: true
    },
    paymentMethod: {
    ...
    cancellations: [
      {
        reason: { type: String },
        status: { type: String, enum: ['pending', 'accepted', 'declined', 'admin_review'], default: 'pending' },
        requestedBy: { type: ObjectId, ref: 'User', required: true },
        requestedAt: { type: Date, default: Date.now },
        decidedBy: { type: ObjectId, ref: 'User' },
        decidedAt: { type: Date },
...
orderSchema.post('save', async function(doc) {
  ...
  if (this.isModified('status') && doc.status === 'complete') {
    try {
      ...
      console.log(`✅ Balance added to user ${doc.createdBy} for completed order ${doc._id}`);
    } catch (error) {
