...
const createGig = {
  body: Joi.alternatives().try(
    ...
    Joi.object().keys({
      ...
      packages: Joi.object().keys({
        ...
        standard: Joi.object().keys({
          ...
          features: Joi.array().items(Joi.string().max(100)).default([]).optional(),
          duration: Joi.string().allow('').optional(),
          instrument: Joi.string().allow('').optional()
        }).optional(),
      ...
      referenceSongs: Joi.array().items(Joi.string()).max(4).optional().default([]),
      referenceArtworks: Joi.array().items(Joi.string()).max(4).optional().default([]),
      videoUrl: Joi.string().uri().allow('').optional(),
      audioSamples: Joi.array().items(Joi.object().keys({
      ...
      faq: Joi.array().items(Joi.object().keys({
        question: Joi.string().required().max(100),
        answer: Joi.string().required().max(300)
      })).max(10).optional().default([]),
      gig_extras: Joi.array().items(Joi.object().keys({
        title: Joi.string().required().max(50),
        description: Joi.string().max(100),
        price: Joi.number().required().min(5).max(1000),
        additionalTime: Joi.number().min(0).max(15)
      })).max(5).optional().default([]),
      searchKeywords: Joi.array().items(Joi.string().trim()).max(10).optional().default([]),
      status: Joi.string().valid('active', 'paused', 'draft', 'under_review').optional().default('draft'),
    ...
    Joi.object().keys({
      ...
      status: Joi.string().valid('active', 'paused', 'draft', 'under_review').default('draft'),
...
const getGigs = {
  query: Joi.object().keys({
    ...
    country: Joi.string(),
    language: Joi.string(),
    sortBy: Joi.string().valid('newest', 'oldest', 'price_low', 'price_high', 'rating', 'popular'),
...
const updateGigStatus = {
  ...
  body: Joi.object().keys({
    status: Joi.string().required().valid('draft', 'active', 'paused', 'denied'),
